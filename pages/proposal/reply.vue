<template>
	<view class="container">
		<view class="tabBox flex">
			<view class="tab full" :class="{'active-1': type == 1}" @click="changeTab(1)">待回复</view>
			<view class="tab full" :class="{'active-2': type == 2}" @click="changeTab(2)">已回复</view>
		</view>
			...
			<load-refresh ref="loadRefresh" :isRefresh="true" :refreshTime="800" :heightReduce="40" :backgroundCover="'#ffffff'"
			 :pageNo="currPage" :totalPageNo="totalPage" @loadMore="loadMore" @refresh="refresh">
				<view slot="content-list">
					<div class="con" v-for="(item,index) in dataList" :key="index" @click="$tools.goToPage('replyDetail?done=' + (type==1?'0':'1'))">
						<div class="content">
							<view class="topBox clearfix">
								<div class="til text-1">
									{{item.title}}
								</div>
								<div class="tip">{{type==1? '待回复' : '已回复'}}</div>
							</view>
